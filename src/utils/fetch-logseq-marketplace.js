...
async function fetchPackages() {
  ...
  if (!res.ok) {
    ...
    console.error(`Failed to fetch package list. Status: ${res.status} ${res.statusText}. Body: ${errorText}`);
...
async function main() {
  try {
    ...
    fs.writeFileSync('logseq-marketplace-plugins.json', JSON.stringify(results, null, 2));
    console.log('Fetched', results.length, 'plugins. Output: logseq-marketplace-plugins.json');
  } catch (e) {
    console.error(e);
  }
}
